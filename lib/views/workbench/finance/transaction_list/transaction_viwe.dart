...
class TransactionView extends GetView<TransactionController> {
  ...
  Widget buildItem(dynamic item) {
    return GestureDetector(
      onTap: () {
        Routers.push(Routers.transactionDetail, {
          'id': item.id,
        });
      },
      child: Container(
          ...
          children: [
            ...
            Row(
              children: [
                AppText(
                  text:
                      '${item.type == 1 ? '+' : '-'} ${item.amount.toString()}',
                  color: item.type == 1 ? AppStyles.primary : AppStyles.textRed,
                  fontWeight: FontWeight.bold,
                  fontSize: 16.sp,
                ),
                SizedBox(width: 4.w),
                AppText(
                  text: controller.currencyModel['code'].toString(),
                  color: item.type == 1 ? AppStyles.primary : AppStyles.textRed,
                  fontWeight: FontWeight.bold,
                  fontSize: 16.sp,
                ),
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _buildLabelText('请输入外部交易号'.tr),
                      5.verticalSpace,
                      BaseInput(
                        hintText: '请输入'.tr,
                        controller: controller.outSerialNoController,
                      ),
                      10.verticalSpace,
  ...
  Widget _buildFilterDateRange(TransactionController controller) {
